...
driver = DMRGDriver(scratch="/central/scratch/hczhai/h2d-su2-%02d" % fid,
    restart_dir="/central/scratch/hczhai/crit/2d-19/%02d-mps" % fid,
    symm_type=SymmetryTypes.SU2, stack_mem=300 << 30,
    n_threads=64, n_mkl_threads=8, mpi=True)

bond_dims = [14000] * 2 + [17000] * 2 + [20000] * 2 + [24000] * 2 + [28000] * 2 + [32000] * 4
noises = [1E-7] * 12 + [0] * 2
thrds = [1E-6] * 14
n_sweeps = 14
...
energy = driver.dmrg(mpo, ket, n_sweeps=n_sweeps, bond_dims=bond_dims, noises=noises, thrds=thrds,
    ...
    dav_max_iter=50, dav_def_max_size=4, forward=False, sweep_start=11)
print('DMRG energy = %20.15f' % energy)

dm = driver.get_1pdm(ket, iprint=2, max_bond_dim=32000)
if driver.mpi.rank == 0:
    np.save("%02d-1pdm.npy" % fid, dm)

driver.mpi.barrier()
dm = driver.get_npdm(ket, pdm_type=2, npdm_expr='((C+D)2+(C+D)2)0', mask=(0, 0, 1, 1), iprint=2, max_bond_dim=32000)
dm = dm * (0.5 * -np.sqrt(3) / 2)
if driver.mpi.rank == 0:
    np.save("%02d-corr.npy" % fid, dm)

driver.mpi.barrier()
dm = driver.get_npdm(ket, pdm_type=2, npdm_expr='((C+D)0+(C+D)0)0', mask=(0, 0, 1, 1), iprint=2, max_bond_dim=32000)
dm = dm * 2
if driver.mpi.rank == 0:

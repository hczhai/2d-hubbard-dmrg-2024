...
driver = DMRGDriver(scratch="/central/groups/changroup/scratch/hczhai/h2d-su2-%02d" % fid,
    restart_dir="/central/scratch/hczhai/crit/2d-19/%02d-mps-x1" % fid,
    symm_type=SymmetryTypes.SU2, stack_mem=300 << 30,
    n_threads=64, n_mkl_threads=8, mpi=True)

bond_dims = [20000] * 2 + [24000] * 2 + [28000] * 2 + [32000] * 4
noises = [1E-7] * 8 + [0] * 2
thrds = [1E-6] * 10
n_sweeps = 10
...
energy = driver.dmrg(mpo, ket, n_sweeps=n_sweeps, bond_dims=bond_dims, noises=noises, thrds=thrds,
    ...
    dav_max_iter=50, dav_def_max_size=4, forward=False, sweep_start=7)
print('DMRG energy = %20.15f' % energy)

dm = driver.get_1pdm(ket, iprint=2, max_bond_dim=32000)
if driver.mpi.rank == 0:
    np.save("%02d-1pdm-x1.npy" % fid, dm)

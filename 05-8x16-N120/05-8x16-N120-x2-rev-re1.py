...
driver = DMRGDriver(scratch="/central/groups/changroup/scratch/hczhai/h2d-su2-%02d-rev" % fid,
    restart_dir="/central/scratch/hczhai/crit/2d-19/%02d-mps-x2-rev" % fid,
...
energy = driver.dmrg(mpo, ket, n_sweeps=n_sweeps, bond_dims=bond_dims, noises=noises, thrds=thrds,
    ...
    dav_def_max_size=4, forward=False, sweep_start=6)
print('DMRG energy = %20.15f' % energy)

dm = driver.get_1pdm(ket, iprint=2, max_bond_dim=16000)
if driver.mpi.rank == 0:
    np.save("%02d-1pdm-x2-rev.npy" % fid, dm)
